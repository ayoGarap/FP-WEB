...
router.post('/', upload.single('image'),async function(req, res, next) {
  try {
    ...
    result = await aboutMeViewSetting.addAboutMeViewSetting(req.body, imagePath);
    res.json(await result[0]);
...
router.put('/:idAboutMeView', upload.single('image'), async function(req, res, next) {
  try {
    console.log(req.body)
    if (req.file == undefined){
      imagePath = undefined;
    ...
    result = await aboutMeViewSetting.updateAboutMeViewSetting(req.body,req.params.idAboutMeView,imagePath);
    res.json(await result[0]);
  } catch (err) {
